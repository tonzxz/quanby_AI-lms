...
interface StatsData {
  ...
  totalClasses: number;
  totalAssessments: number; // Add this property
  newAssessments: number;   // Add this property
}
@Component({
  selector: 'app-thome',
  ...
  generateCalendar(month: number, year: number): void {
    ...
    for (let i = 1; i <= days_of_month[month]; i++) {
      ...
      if (
        ...
        day.innerHTML = ` <span class="text-xs text-white bg-blue-500 rounded-full p-1.5 ml-1"> ${i} </span>`;
      }
